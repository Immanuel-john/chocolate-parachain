...
pub mod pallet {
	...
	use frame_support::{
		...
		pallet_prelude::*,
		sp_runtime::{traits::{CheckedDiv,Saturating}, ArithmeticError},
		traits::{
	...
	pub type ReviewAl<T> = Review<<T as frame_system::Config>::AccountId, <T as Config>::StringLimit>;
	/// type alias for project
	pub type ProjectAl<T> = Project<<T as frame_system::Config>::AccountId, BalanceOf<T>,<T as Config>::StringLimit>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_project(origin: OriginFor<T>, project_meta: BoundedVec<u8, T::StringLimit>) -> DispatchResult {
		...
		pub fn initialize_reviews(
			...
			let list_of_revs: Vec<ReviewAl<T>> = constants::project::REVS
				...
				.map(|(rev, id)| {
					...
					let rev_vec: BoundedVec<_,T::StringLimit> = rev.to_vec().try_into().unwrap();
					// init rev
					let review: ReviewAl<T> = Review{
						project_id: count,
						proposal_status: ProposalStatus{
	...
	pub struct GenesisConfig<T: Config> {
		...
		pub init_projects: Vec<(T::AccountId, Status, ReasonOf<T>)>,
	}
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> 
		...
		fn build(&self) {
			...
			let meta: Vec<BoundedVecOf<u8,T>> =
				constants::project::METADATA.iter().map(|each| each.to_vec().try_into().ok().unwrap()).collect();
